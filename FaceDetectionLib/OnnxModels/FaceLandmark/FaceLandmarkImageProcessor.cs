...
internal class FaceLandmarkImageProcessor : SkiaSharpImageProcessor<FaceLandmarkPrediction, float>
{
    const int RequiredWidth = 112;
    const int RequiredHeight = 112;
    ...
    protected override Tensor<float> OnGetTensorForImage(SKBitmap image)
    {
        ...
        // For Ultraface, the expected input would be 112 x 112 x 4 (in RGBA format)
        var expectedInputLength = RequiredWidth * RequiredHeight * 4;

        // For the Tensor, we need 3 channels so 112 x 112 x 3 (in RGB format)
    ...
    protected override void OnApplyPrediction(FaceLandmarkPrediction prediction, SKPaint textPaint, SKPaint rectPaint, SKCanvas canvas)
        => canvas.DrawPoints(SKPointMode.Points, prediction.Points, rectPaint);
}
